...
static long _gif_skip_chunk(uint8_t **buff, long size) {
  long skip;
...
/** [ internal function, do not use ] **/
static long _gif_load_header(unsigned gflg, uint8_t **buff, void **rpal,
...
/** [ internal function, do not use ] **/
static long _gif_load_frame(uint8_t **buff, long *size, uint8_t *bptr) {
...
static long gif_load(void *data, long size, void (*gwfr)(void*, GIF_WHDR*),
  ...
  buff = (uint8_t*)(ghdr + 1) /** skipping the global header and palette **/
     + _gif_load_header(ghdr->flgs, 0, 0, 0, 0, 0L) * 3L;
  ...
  for (whdr.bptr = buff, whdr.bkgd = ghdr->bkgd, blen = --size;
     ...
     blen = _gif_skip_chunk(&whdr.bptr, blen) - 1) /** count all frames **/
    if (desc == GIF_FHDM) {
      fhdr = (struct GIF_FHDR*)whdr.bptr;
      if (_gif_load_header(ghdr->flgs, &whdr.bptr, (void**)&whdr.cpal,
